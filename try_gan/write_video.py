...
def write_video(frames, path, rate=30.0, size=(256, 256)):
    ...
    out = cv2.VideoWriter("{}.mp4".format(path), cv2.VideoWriter_fourcc(*'MP4V'), rate, (frame_width, frame_height))
...
def batch_videos(frames, total_frame_count, path, chunk_frame_count=3600, rate=30.0, size=(256, 256)):
    def frame_chunk():
        for i, frame in enumerate(frames):
            yield frame
